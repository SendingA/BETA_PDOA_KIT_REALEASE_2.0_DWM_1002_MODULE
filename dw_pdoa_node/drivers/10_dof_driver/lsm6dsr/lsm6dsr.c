...
#define NRF_LOG_MODULE_NAME LSM6DSR
...
static struct lsm6dsr_cfg _cfg = LSM6DSR_CFG_DEFAULTS;

static uint8_t read_bytes(uint8_t address, uint8_t *buffer, uint16_t length)
{
    ret_code_t ret = 0;
...
static uint8_t write_bytes(uint8_t address, uint8_t *buffer, uint16_t length)
{
    ret_code_t ret = 0;
...
static uint8_t write_byte(uint8_t address, uint8_t byte)
{
...
static void lsm6dsm_reset()
{
    // reset device
    uint8_t temp;
    read_bytes(LSM6DSR_CTRL3_C, &temp, 1);
    temp |= 0x01;
    write_bytes(LSM6DSR_CTRL3_C, &temp, 1); // Set bit 0 to 1 to reset LSM6DSM
    nrf_delay_ms(10); // Wait for all registers to reset 
}

void lsm6dsr_init(struct lsm6dsr_cfg *cfg)
{
    memcpy(&_cfg, cfg, sizeof(struct lsm6dsr_cfg));
    ...
    write_byte(LSM6DSR_CTRL1_XL,(LSM6DSR_AODR_6660Hz << 4) | (LSM6DSR_AFS_16G<<2) | 2 );
  
    _cfg.gyro_sensitivity = 2000;
    write_byte(LSM6DSR_CTRL2_G, (LSM6DSR_GODR_6660Hz << 4) | (LSM6DSR_GFS_2000DPS<<2));

    write_byte(LSM6DSR_CTRL3_C, 0x04); 
    read_bytes(LSM6DSR_CTRL3_C, &temp, 1);
    // enable block update (bit 6 = 1), auto-increment registers (bit 2 = 1)
    write_byte(LSM6DSR_CTRL3_C, temp| 0x40 | 0x04); 
    ...
    write_byte(LSM6DSR_CTRL8_XL, 0x20 );

    // interrupt handling
//    write_byte(LSM6DSM_DRDY_PULSE_CFG, 0x80); // latch interrupt until data read
    write_byte(LSM6DSR_COUNTER_BDR_REG1, 0x80); // pulsed interrupt mode
    write_byte(LSM6DSR_INT1_CTRL, 0x03);      // enable accel/gyro interrupts on INT1
}


static uint8_t readRawData(int16_t *acc_gyro)
{    
    int16_t rawdata[7];
    uint8_t r = read_bytes(LSM6DSR_OUT_TEMP_L, (uint8_t*)rawdata, 14);
    for (int i=0;i<7;i++)
...
void lsm6dsr_offsetBias(float * gyro_bias, float * accel_bias)
{
...
int32_t lsm6dsr_hwtest()
{
    ...
    uint8_t r = read_bytes(LSM6DSR_WHO_AM_I, &value, 1);
    if (r) return r;
    if (value != LSM6DSR_WHO_AM_I_VALUE)
    {
        NRF_LOG_ERROR("HWTEST: %u, got=%X, expected=%X", r, value,
                     LSM6DSR_WHO_AM_I_VALUE);

    }
    return (value == LSM6DSR_WHO_AM_I_VALUE) ? NRF_SUCCESS : 1;
}
